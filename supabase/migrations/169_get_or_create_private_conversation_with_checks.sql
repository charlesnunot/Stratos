-- Extend get_or_create_private_conversation: do profile + blocked checks inside RPC
-- so the client only needs one round-trip (faster, fewer timeouts/aborts).
...
COMMENT ON FUNCTION public.get_or_create_private_conversation(uuid) IS
  'Returns existing private conversation id or creates one. Checks other user status and block; caller = auth.uid(), other = p_other_id. Single round-trip.';
