-- 修复产品分享通知问题
-- 增强错误处理和调试日志，确保产品分享时能正确发送通知给卖家
-- 参考 create_favorite_notification() 的实现方式

-- 重新创建分享通知触发器函数，添加调试日志和改进错误处理
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'trigger_create_share_notification' 
    AND tgrelid = 'shares'::regclass
  ) THEN
    RAISE EXCEPTION 'Trigger trigger_create_share_notification was not created';
  END IF;
  
  RAISE NOTICE 'Trigger trigger_create_share_notification verified: exists and enabled';
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Function create_share_notification verified: SECURITY DEFINER = %', has_security_definer;
END $$;
