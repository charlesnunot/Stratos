-- Exclude internal users from public-facing surfaces (suggested users + public_profiles view).
-- Internal users remain visible to themselves and admin via profiles table / RLS.

SET search_path = public;

-- 1) suggested_users: do not recommend internal users
...
BEGIN
  ...
  mutual AS (
    ...
    WHERE f.follower_id = p_profile_user_id
      ...
      AND p.status = 'active'
      AND (p.user_origin IS NULL OR p.user_origin = 'external')
...
COMMENT ON FUNCTION public.suggested_users(UUID, INT, UUID) IS
  'Suggested users for profile page. Excludes internal users (user_origin=internal).';

-- 2) public_profiles: only expose external or legacy (null user_origin) profiles
...
SELECT
  ...
  created_at
FROM profiles
WHERE user_origin IS NULL OR user_origin = 'external';

COMMENT ON VIEW public_profiles IS 'Public view of profiles (safe fields only). Excludes internal users. Internal users and admin use profiles table.';
