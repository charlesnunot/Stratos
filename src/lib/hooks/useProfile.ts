...
export interface Profile {
  ...
  created_at: string
  status?: string | null
  // email 没有落在 profiles 表结构里，先不从这里读取，避免列不存在报错
  email?: string | null
  tip_enabled?: boolean | null
  // Profile edit pending fields
  profile_status?: 'active' | 'pending' | null
  ...
  // Subscription fields
  seller_type?: 'direct' | 'external' | null
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const selectFields = isOwnProfile
        ? 'id, username, display_name, avatar_url, bio, location, follower_count, following_count, created_at, status, subscription_type, role, tip_enabled, seller_type'
